...
1. Search for `'rg_wp_base'` (inside single quotations) to capture the text domain.
2. Search for `rg_wp_base_` to capture all the function names.
3. Search for ` rg_wp_base` (with a space before it) to capture DocBlocks.
4. Search for `rg_wp_base-` to capture prefixed handles.

OR

* Search for: `'rg_wp_base'` and replace with: `'megatherium'`
* Search for: `rg_wp_base_` and replace with: `megatherium_`
